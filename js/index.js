...
var urlWholeBIDS = "http://" + host + "/geoserver/opengeo/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=opengeo:DATATABLE&srsName=EPSG:4326&outputformat=json";
...
function getGeoJson(data) {
	...
	geoJsonLayerNIV = new L.geoJson(data, {style: StyleNIV});
}
...
function getGeoJsonBIDS(data){
	//geoJsonLayerBIDS = new L.geoJson(data);
	geoJsonLayerBIDS = new L.geoJson(data, {onEachFeature: onEachFeatureBIDS});
	
	markers = L.markerClusterGroup({showCoverageOnHover: false, maxClusterRadius: 30});
	markers.addLayer(geoJsonLayerBIDS);
};

$.ajax({
	url: urlWholeBIDS,
	dataType: 'json',
	jsonpCallback: getGeoJsonBIDS,
	success: getGeoJsonBIDS
});
...
function StyleNIV(feature) {
	return {
		weight: 1,
		opacity: 1,
		color: 'white',
		fillOpacity: 0.7,
		fillColor: getColorNIV(feature.properties.NIV_2013)
	};
}
...
function getCurrentKey(){
	...
	{
	keyWCC();
	} else if (currentKey=="NIV")
	{
	keyNIV();
	} else if (currentKey=="BIDS")
	{
	keyBIDS();
	}
}
...
keyTP = function () {
	...
	var categoryDescription = "<div><h7 class='lorem'>The State Department, along with the Departments of Commerce, Treasury and Agriculture, works to open new export opportunities for American business, farmers, ranchers and other workers.  Free trade agreements with other countries widen access for U.S. goods and services abroad.  Trade partnerships also support global economic development, resolve trade disputes, and strengthen intellectual property rights enforcement.</h7></div>";
...
keyMLO = function () {
	...
	var categoryDescription = "<div><h7 class='lorem'>The State Department’s Office of Multilateral Trade Affairs represents  U.S. trade policy in the World Trade Organization (WTO) and the Organization for Economic Cooperation and Development (OECD.)   It provides technical expertise in trade, labor, environment, and government procurement during trade negotiations with other countries.</h7></div>";
...
keyHI = function () {
	...
	var key6Description = "The State Department and the U.S. Agency for International Development have played a vital role in the worldwide campaign against infectious diseases, including HIV/AIDS, tuberculosis, malaria, and polio.  Addressing global health challenges is a moral imperative – and it bolsters global security, fosters political stability and promotes economic growth and development. Through the President’s Emergency Plan for AIDS Relief (PEPFAR), begun during the President George W. Bush Administration, the United States supports treatment for over 1.7 million people around the world, and has enabled 200,000 children to be born HIV-free.  The program was reauthorized in 2008, with an added commitment of $48 billion over five years to combat HIV/AIDS, tuberculosis, and malaria.<br>The United States is on the frontlines of the global battle against tuberculosis, one of the three leading causes of infectious disease deaths worldwide.  The U.S. Agency for International Development (USAID) works with other countries to improve their services to patients and to develop new diagnostic technologies.<br>We also support the fight against malaria through the President’s Malaria Initiative (PMI), a five-year program run by USAID that targets 15 of the hardest-hit countries in Africa.  PMI aims to reduce malaria-related deaths by 50 percent in the focus countries.<br>The United States has provided nearly 30 percent of the global contributions to the Global Polio Eradication Initiative, which has brought the number of countries affected by polio from 125 in 1988 down to four.<br>The State Department’s Global Health Initiative, an investment of $63 billion, places an emphasis on the health of women and girls who have a multiplier effect on their families and communities.";
...
keySTI = function () {
	...
	var key1LayerPts = "";
	var key1Description = "Science, technology and innovation are central to America’s 21st century diplomacy. The State Department promotes international scientific cooperation to foster worldwide economic and social development and to create innovative solutions to global challenges.  The Office of the Science and Technology Adviser to the Secretary of State provides scientific and technical advice and resources to bureaus and offices at the Department of State.";
...
keyECC = function () {
	...
	var key1LayerPts = "";
	var key1Description = "Climate change is one of the world’s most urgent, complex and far-reaching problems.  It threatens the future in countless ways – including causing natural disasters and impacting both global security and economic growth.  The United States is taking a leading role in addressing climate change domestically and internationally through an ever expanding range of measures.  We are, among other things, reducing our emissions at home, developing transformational low-carbon technologies, and improving our observations systems to help us understand and address the possible impacts of climate change.";
...
keyWCC = function () {
	...
	var key1LayerPts = "";
	var key1Description = "The U.S. Agency for International Development’s Office of U.S. Foreign Disaster Assistance (OFDA) and the State Department’s Bureau of Consular Affairs (CA) are important players when disaster strikes abroad.<br>OFDA responds to an average of 70 disasters worldwide every year, including earthquakes, volcanoes, floods, drought and conflict.   Its experts worldwide and in Washington, D.C. help countries prepare for, respond to, and recover from humanitarian crises.<br>When disasters happen, OFDA sends regional and technical experts to the affected country to identify humanitarian needs.   In the case of a large-scale disaster, OFDA can deploy a Disaster Assistance Response Team (DART) to manage the U.S. response, while working closely with local officials, the international community and relief agencies.  OFDA also maintains stocks of emergency relief supplies in warehouses worldwide and can deliver them quickly.  After a disaster, it supports programs that give people tools to restart former jobs, provide care to traumatized disaster survivors, and prepare individuals to get back on their feet.<br>The State Department’s Bureau of Consular Affairs (CA) and the U.S. embassy in an affected country help Americans caught in a disaster or crisis overseas.   CA urges U.S. citizens overseas to register with its Smart Traveler Enrollment Program to receive situation updates and useful advice when needed. In major disasters, CA may urge U.S. citizens to leave the country, and if commercial transportation is not available, may provide departure assistance, as resources permit.";
...
keyNIV = function () {
	...
	var key1Subject = "Non-Immigrant Visas (2013)";
	var key1Layer = "geoJsonLayerNIV";
	var key1LayerPts = "";
	var key1Description = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam aliquet fermentum ipsum, id commodo orci dignissim non. Mauris vulputate ultricies leo, et porta orci pretium in. Duis pulvinar iaculis augue, sit amet mollis quam tristique.";
	
	var key1div = L.DomUtil.create('div'),
		grades = ['99999', '49999', '24999', '9999', '4999', '0'],
		// this is something like a subheader
		key1Labels = [],
		from;

	for (var i = 0; i < grades.length; i++) {
		from = ['100,000 or more', '50,000 to 99,999', '25,000 to 49,999', '10,000 to 24,999', '5,000 to 9,999', 'Less than 5,000'];
		
		key1Labels.push(
			'<i style="background:' + getColorNIV(grades[i]) + '"></i>' +
			from[i]
		);
	}
...
keyBIDS = function () {
	...
	var key1Subject = "Increasing Markets";
	var key1Layer = "geoJsonLayerBIDS";
	var key1LayerPts = "";
	var key1Description = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam aliquet fermentum ipsum, id commodo orci dignissim non. Mauris vulputate ultricies leo, et porta orci pretium in. Duis pulvinar iaculis augue, sit amet mollis quam tristique.";
	...
	for (var i = 0; i < grades.length; i++) {
		...
		key1Labels.push(
			'<i style="background:' + getColorBIDS(grades[i]) + '"></i>' +
			from[i]
		);
	}
...
function onEachFeatureBIDS(feature, layer) {
	layer.on({
	//mouseover: highlightFeature,
	//mouseout: resetHighlight
	//click: zoomToFeature
